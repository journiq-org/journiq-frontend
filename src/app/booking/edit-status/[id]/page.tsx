...
//     try {
//       await dispatch(
//         updateBookingStatus({ bookingId: id as string, status })
//       ).unwrap();
//       router.push("/booking/guide-booking");
//     } catch (error) {
//       console.error("Failed to update booking status:", error);
...
//     if (status === "completed") {
//       return optionValue !== "completed"; // Only allow completed
//     }
//     if (status === "confirmed") {
//       return !["confirmed", "completed"].includes(optionValue); // Only confirmed + completed
//     }
//     if (status === "cancelled") {
//       return optionValue !== "cancelled"; // Only cancelled
//     }
//     return false; // pending → nothing disabled
//   };
...
//                 Modify the status of this booking
//               </p>
//             </div>
//           </div>
//         </div>
...
//               {selectedStatusOption && (
//                 <div
//                   className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border ${selectedStatusOption.bgColor} ${selectedStatusOption.borderColor}`}
//                 >
//                   <selectedStatusOption.icon
//                     className={`w-4 h-4 ${selectedStatusOption.color}`}
//                   />
//                   <span
//                     className={`text-sm font-medium ${selectedStatusOption.color}`}
//                   >
//                     {selectedStatusOption.label}
//                   </span>
//                 </div>
...
//                     <button
//                       key={option.value}
//                       onClick={() => !disabled && setStatus(option.value)}
//                       disabled={disabled}
//                       className={`p-4 rounded-xl border-2 transition-all duration-200 text-left ${
//                         isSelected
//                           ? `${option.bgColor} ${option.borderColor} border-opacity-100 shadow-sm`
...
//             <div className="flex items-center justify-between">
//               <p className="text-sm text-slate-600">
//                 Make sure to review the booking details before updating the
//                 status.
//               </p>
...
//               <ul className="text-sm text-blue-800 space-y-1">
//                 <li>
//                   • <strong>Confirmed:</strong> Booking has been accepted and
//                   confirmed
//                 </li>
//                 <li>
//                   • <strong>Completed:</strong> Tour has been successfully
//                   finished
//                 </li>
//                 <li>
//                   • <strong>Cancelled:</strong> Booking has been cancelled
//                 </li>
//                 <li>
//                   • <strong>Pending:</strong> Booking is waiting for confirmation
//                 </li>
//               </ul>
//             </div>
//           </div>
...
const EditStatusPage = () => {
  ...
  const router = useRouter();
  const dispatch = useAppDispatch();
  const [status, setStatus] = useState("pending"); // ✅ Default set as pending
  const [isUpdating, setIsUpdating] = useState(false);
  ...
  // 🔹 Disable logic
  const isOptionDisabled = (optionValue: string) => {
    if (status === "completed") return optionValue !== "completed"; // Only completed stays active
    if (status === "confirmed") return !["confirmed", "completed"].includes(optionValue); // Allow confirmed + completed
    if (status === "cancelled") return optionValue !== "cancelled"; // Only cancelled stays active
    return false; // For pending → allow everything
  };
                ...
                >
                  {statusOptions.map((option) => (
                    <option
                      key={option.value}
                      value={option.value}
                      disabled={isOptionDisabled(option.value)} // ✅ Disable logic
                    >
                      {option.label}
                    </option>
                  ))}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p
                            className={`text-sm font-medium ${
                              isSelected ? option.color : "text-slate-900"
                            }`}
                          >
                            {option.label}
                          </p>
