...
// // ✅ Validation schema - removed is_active since it's automatic
...
//   lat?: number | null;
//   lng?: number | null;
//   // Removed is_active - it's automatic
// };
...
//     // Removed defaultValues for is_active since it's automatic
//   });
...
//       ? data.popularAttractionsRaw.split(",").map((s) => s.trim()).filter(s => s.length > 0)
//       : [];
//     formData.append("popularAttractions", JSON.stringify(popularAttractions));

//     const tags = data.tagsRaw
//       ? data.tagsRaw.split(",").map((s) => s.trim()).filter(s => s.length > 0)
//       : [];
...
//     // Location as JSON string - only if both lat and lng are provided
//     if (data.lat !== null && data.lng !== null && data.lat !== undefined && data.lng !== undefined) {
...
// type DestinationFormData = yup.InferType<typeof schema>;
...
//   const addItem = (array: string[], setArray: React.Dispatch<React.SetStateAction<string[]>>, fieldName: string) => {
//     const newArray = [...array, ''];
//     setArray(newArray);
//     setValue(fieldName as any, newArray);
//   };

//   const removeItem = (index: number, array: string[], setArray: React.Dispatch<React.SetStateAction<string[]>>, fieldName: string) => {
//     if (array.length > 1) {
...
//   const updateItem = (index: number, value: string, array: string[], setArray: React.Dispatch<React.SetStateAction<string[]>>, fieldName: string) => {
//     const newArray = [...array];
...
//     formData.append("popularAttractions", JSON.stringify(data.popularAttractions));

//     // Filter out undefined values and send as JSON strings
//     const filteredTags = data.tags?.filter((item): item is string => item !== undefined && item.trim() !== "") || [];
...
//       router.push('/admin-dashboard/destinations');
//     } catch (err: any) {
//       console.error("❌ Destination creation failed:", err);
...
//             </button>
//             <div>
//               <h1 className="text-3xl font-bold text-slate-900">Create New Destination</h1>
//               <p className="text-slate-600 mt-1">Add a new destination for guides to create tours</p>
//             </div>
//           </div>
//         </div>
//       </div>
...
export default function CreateDestinationPage() {
  ...
  const onSubmit = async (data: DestinationFormData) => {
    ...
    try {
      ...
      // Arrays - send as JSON strings to match backend expectations
      const filteredAttractions = data.popularAttractions.filter(attr => attr.trim() !== '');
      formData.append("popularAttractions", JSON.stringify(filteredAttractions));

      // Filter out undefined values and send as JSON strings
      const filteredTags = data.tags.filter(tag => tag !== undefined && tag.trim() !== "");
      formData.append("tags", JSON.stringify(filteredTags));
