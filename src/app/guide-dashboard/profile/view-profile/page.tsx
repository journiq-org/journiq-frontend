...
const [openDialog, setOpenDialog] = useState(false);
  ...
  const { guideTours, isLoading: toursLoading } = useAppSelector((state: any) => state.tour);
  const { bookings, loading: bookingsLoading } = useAppSelector((state: any) => state.guideBookings);
...
useEffect(() => {
  ...
  const tourStats = {
    totalTours: guideTours?.length || 0,
  };
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-4 bg-blue-50 rounded-lg">
                        <div className="text-2xl font-bold text-blue-600">
                           {toursLoading ? "Loading..." : tourStats.totalTours}
                        </div>
                        <div className="text-sm text-slate-600">Total Tours</div>
                      </div>
                      <div className="text-center p-4 bg-green-50 rounded-lg">
                        <div className="text-2xl font-bold text-green-600"> 
                          {bookingsLoading ? "Loading..." : bookingStats.totalBookings}
                          </div>
...
//       }
//       dispatch(fetchUserProfile());
//       dispatch(guideViewTours({ page: 1, limit: 100 })); // fetch all tours for stats
//       dispatch(fetchGuideBookings()); // fetch all bookings for stats
//     };
...
//               </div>
//               <div className="flex-1 text-center md:text-left">
//                 <h1 className="text-3xl font-bold text-white mb-2">{profile?.name || "Guide"}</h1>
//                 <p className="text-blue-100 mb-4">Professional Tour Guide</p>
//                 <button
//                   onClick={() => router.push("/guide-dashboard/profile/edit-profile")}
